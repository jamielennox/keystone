...
class CryptoManager(utils.SingletonManager):
    ...
    def extract(self, key, rnd_data=None):
        if not rnd_data:
            rnd_data = self.new_key(self.KEY_SIZE)
    ...
    def get_storage_keys(self, name):
        ...
        """
        if not self.mkey:
            raise exception.CryptoError('Failed to find mkey')
    ...
    def encrypt_key(self, name, key):
        ...
        try:
            ...
            raise exception.CryptoError('Failed to encrypt key')

        # sign it for integrity
        try:
            signature = self.crypto.sign(skey, enc_key, b64encode=False)
        except Exception:
            raise exception.CryptoError('Failed to sign key')
    ...
    def decrypt_key(self, name, enc_key, signature):
        ...
        try:
            ...
            raise exception.CryptoError('Failed to verify key')

        if not sigc == signature:
            raise exception.CryptoError('Signature check failed')

        # decrypt the key
        try:
            plain = self.crypto.decrypt(ekey, enc_key, b64decode=False)
        except Exception:
            raise exception.CryptoError('Failed to decrypt key')
