...
class KeyApiTests(base.BaseTestCase):
    ...
    def test_key_setting(self):
        resp = self.put('key/%s' % DEFAULT_REQUESTOR,
                        expected_status=200,
    ...
    #     self.put('key', expected_status=404)
    #     self.put('key/%s' % DEFAULT_REQUESTOR, expected_status=401)
    #     self.put('key/%s' % DEFAULT_REQUESTOR, expected_status=401,
    #              json={'fake': 'values'})

    def test_override_key(self):
        resp1 = self.put('key/%s' % DEFAULT_REQUESTOR,
                         expected_status=200,
                         json={'key': base64.b64encode(REQUEST_KEY)})
        ...
        resp2 = self.put('key/%s' % DEFAULT_REQUESTOR,
                         expected_status=200,
